...
import { conditionalCsrf, csrfProtection, getCsrfToken, csrfErrorHandler } from "./middleware/csrf";
...
if (!envValidation.isValid && process.env.NODE_ENV === 'production') {
  console.error('⛔ Cannot start in production with critical environment errors');
  process.exit(1);
}
...
// Create and export session middleware for sharing with Socket.IO
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://cdn.jsdelivr.net", "https://fonts.googleapis.com", "https://cdn.paddle.com", "https://apis.google.com", "https://accounts.google.com", "https://www.gstatic.com"],
      styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://accounts.google.com"],
      imgSrc: ["'self'", "data:", "https:", "blob:"],
      fontSrc: ["'self'", "data:", "https://fonts.gstatic.com"],
      connectSrc: ["'self'", "ws:", "wss:", "https://api.paddle.com", "https://sandbox-api.paddle.com", "https://apis.google.com", "https://accounts.google.com", "https://www.googleapis.com", "https://securetoken.googleapis.com", "https://identitytoolkit.googleapis.com", "https://firestore.googleapis.com", "https://*.firebaseio.com", "https://*.cloudfunctions.net"],
      frameSrc: ["'self'", "https://checkout.paddle.com", "https://sandbox-checkout.paddle.com", "https://accounts.google.com", "https://www.google.com", "https://*.firebaseapp.com", "https://*.firebaseio.com"],
      objectSrc: ["'none'"],
...
// Session middleware MUST be applied before CSRF (CSRF uses session storage)
// Applied here (before async block) so it's available to all routes
app.use(sessionMiddleware);

// Apple domain verification for Sign in with Apple (must be before Vite)
app.get('/.well-known/apple-developer-domain-association.txt', (_req, res) => {
  res.type('text/plain').send('lilove.org');
});

// Create HTTP server IMMEDIATELY for fast health check response
const httpServer = createServer(app);

// CRITICAL: Health check routes MUST be registered FIRST - before any middleware
// These routes are needed for Replit deployment health probes (10s timeout)
...
async function initializeApp() {
  ...
  // CSRF token endpoint - needs csrfProtection to make req.csrfToken() available
  (app.get as Function)('/api/csrf-token', csrfProtection, getCsrfToken);
  
  // CSRF protection middleware (session-based)
  // Skips webhooks, OAuth callbacks, and GET requests
  app.use(conditionalCsrf);
  
  // Register routes and setup auth - this is the heavy part
  await registerRoutes(app, sessionMiddleware, httpServer);
