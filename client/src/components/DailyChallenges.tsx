...
export default function DailyChallenges() {
  ...
  const { data: streakInfo } = useQuery<GamificationProfile>({
  ...
  const claimLoginMutation = useMutation({
    mutationFn: async (): Promise<DailyLoginResponse> => {
  ...
  const updateProgressMutation = useMutation({
    mutationFn: async ({ challengeId, increment }: { challengeId: string; increment: number }): Promise<ChallengeProgressResponse> => {
  ...
  const spinWheelMutation = useMutation({
    mutationFn: async (): Promise<SpinWheelReward> => {
