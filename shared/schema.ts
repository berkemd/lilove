...
export const users = pgTable("users", {
  ...
  appleId: varchar("apple_id", { length: 255 }).unique(), // Apple Sign-In user identifier
  googleId: varchar("google_id", { length: 255 }).unique(), // Google Sign-In user identifier
  firebaseUid: varchar("firebase_uid", { length: 128 }).unique(), // Firebase Authentication UID
  firstName: varchar("first_name"),
  lastName: varchar("last_name"),
  profileImageUrl: varchar("profile_image_url"),
  
  // Additional fields for LiLove
...
export const userProfiles = pgTable("user_profiles", {
  ...
  // AI Privacy & Transparency Settings (KVKK Compliant)
  aiPersonalizationEnabled: boolean("ai_personalization_enabled").default(true), // Use data for AI recommendations
  aiHistoryRetention: boolean("ai_history_retention").default(true), // Store AI conversation history (30-day retention)
  aiMoodAnalysis: boolean("ai_mood_analysis").default(true), // Detect emotional patterns from interactions
  aiAnonymousAnalytics: boolean("ai_anonymous_analytics").default(false), // Contribute anonymized data for AI improvement
...
// Sub-Goals (Milestones) for Goals
export const subGoals = pgTable("sub_goals", {
  ...
  goalIdIdx: index("sub_goals_goal_id_idx").on(table.goalId),
  statusIdx: index("sub_goals_status_idx").on(table.status),
  orderIdx: index("sub_goals_order_idx").on(table.order),
}));
...
export const paymentTransactions = pgTable("payment_transactions", {
  ...
  // Paddle Data
  paddleTransactionId: varchar("paddle_transaction_id").unique(),
  externalTransactionId: varchar("external_transaction_id"), // Generic external transaction ID
  ...
  completedAt: timestamp("completed_at"), // When payment was successfully completed
}, (table) => ({
  ...
  externalTxIdx: index("payment_transactions_external_tx_idx").on(table.externalTransactionId),
}));

// Processed Webhook Events - For idempotency and replay protection
export const processedWebhookEvents = pgTable("processed_webhook_events", {
  eventId: varchar("event_id", { length: 255 }).primaryKey(), // Paddle event ID
  eventType: varchar("event_type", { length: 100 }).notNull(), // transaction.completed, subscription.created, etc.
  provider: varchar("provider", { length: 50 }).notNull().default("paddle"), // paddle, stripe, etc.
  
  // Event Data
  eventData: json("event_data").$type<any>(), // Full event payload for debugging
  
  // Security
  signatureVerified: boolean("signature_verified").notNull().default(true),
  eventTimestamp: timestamp("event_timestamp").notNull(), // Timestamp from webhook event
  
  // Processing
  processedAt: timestamp("processed_at").notNull().defaultNow(),
  processingStatus: varchar("processing_status", { length: 50 }).default("success"), // success, failed, skipped
  ...
  eventTypeIdx: index("processed_webhook_events_event_type_idx").on(table.eventType),
  providerIdx: index("processed_webhook_events_provider_idx").on(table.provider),
  processedAtIdx: index("processed_webhook_events_processed_at_idx").on(table.processedAt),
  eventTimestampIdx: index("processed_webhook_events_event_timestamp_idx").on(table.eventTimestamp),
}));
...
// ===== MODULAR AVATAR ENGINE =====

// Rarity enum for avatar traits
...
// Unlock source types
export const UNLOCK_SOURCES = ['achievement', 'challenge', 'purchase', 'default', 'event', 'referral'] as const;
export type UnlockSource = typeof UNLOCK_SOURCES[number];

// Initial 25 Avatar Zones for the modular avatar system
export const AVATAR_ZONES = [
  { key: 'skin', name: 'Skin Tone', layerOrder: 1 },
  { key: 'body', name: 'Body Type', layerOrder: 2 },
  { key: 'face_shape', name: 'Face Shape', layerOrder: 3 },
  { key: 'eyes', name: 'Eyes', layerOrder: 4 },
  { key: 'eyebrows', name: 'Eyebrows', layerOrder: 5 },
  { key: 'nose', name: 'Nose', layerOrder: 6 },
  { key: 'mouth', name: 'Mouth', layerOrder: 7 },
  { key: 'ears', name: 'Ears', layerOrder: 8 },
  { key: 'hair', name: 'Hair Style', layerOrder: 9 },
  { key: 'hair_color', name: 'Hair Color', layerOrder: 10 },
  { key: 'facial_hair', name: 'Facial Hair', layerOrder: 11 },
  { key: 'makeup', name: 'Makeup', layerOrder: 12 },
  { key: 'glasses', name: 'Glasses', layerOrder: 13 },
  { key: 'clothing_top', name: 'Top/Shirt', layerOrder: 14 },
  { key: 'clothing_bottom', name: 'Bottom/Pants', layerOrder: 15 },
  { key: 'shoes', name: 'Shoes', layerOrder: 16 },
  { key: 'hat', name: 'Hat/Headwear', layerOrder: 17 },
  { key: 'jewelry', name: 'Jewelry', layerOrder: 18 },
  { key: 'tattoo', name: 'Tattoos', layerOrder: 19 },
  { key: 'scars', name: 'Scars/Marks', layerOrder: 20 },
  { key: 'wings', name: 'Wings', layerOrder: 21 },
  { key: 'aura', name: 'Aura/Glow', layerOrder: 22 },
  { key: 'pet', name: 'Companion Pet', layerOrder: 23 },
  { key: 'background', name: 'Background', layerOrder: 0 },
  { key: 'frame', name: 'Frame/Border', layerOrder: 24 },
] as const;

export type AvatarZoneKey = typeof AVATAR_ZONES[number]['key'];

// Avatar Zones - Customizable categories for modular avatar system
export const avatarZones = pgTable("avatar_zones", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  keyIdx: index("avatar_zones_key_idx").on(table.key),
  layerOrderIdx: index("avatar_zones_layer_order_idx").on(table.layerOrder),
}));

// Avatar Traits - Individual customization options within each zone
export const avatarTraits = pgTable("avatar_traits", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  zoneId: varchar("zone_id").references(() => avatarZones.id, { onDelete: 'cascade' }).notNull(),
  ...
  assetUrlFront: varchar("asset_url_front"),
  assetUrlSide: varchar("asset_url_side"),
  assetUrlBack: varchar("asset_url_back"),
  thumbnailUrl: varchar("thumbnail_url"),
  
  layerOrder: integer("layer_order").default(0),
  
  unlockType: varchar("unlock_type").default("purchase"),
  ...
  zoneIdx: index("avatar_traits_zone_idx").on(table.zoneId),
  rarityIdx: index("avatar_traits_rarity_idx").on(table.rarity),
  isDefaultIdx: index("avatar_traits_is_default_idx").on(table.isDefault),
  isActiveIdx: index("avatar_traits_is_active_idx").on(table.isActive),
  unlockTypeIdx: index("avatar_traits_unlock_type_idx").on(table.unlockType),
}));

// User Avatar Traits - Tracks which traits users have unlocked
export const userAvatarTraits = pgTable("user_avatar_traits", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  traitId: varchar("trait_id").references(() => avatarTraits.id, { onDelete: 'cascade' }).notNull(),
  
  unlockedAt: timestamp("unlocked_at").defaultNow(),
  unlockSource: varchar("unlock_source").default("purchase"),
  unlockSourceId: varchar("unlock_source_id"),
  
  coinsPaid: integer("coins_paid").default(0),
  
  createdAt: timestamp("created_at").defaultNow(),
}, (table) => ({
  userIdx: index("user_avatar_traits_user_idx").on(table.userId),
  traitIdx: index("user_avatar_traits_trait_idx").on(table.traitId),
  unlockSourceIdx: index("user_avatar_traits_unlock_source_idx").on(table.unlockSource),
  uniqueUserTrait: unique().on(table.userId, table.traitId),
}));

// User Avatar Equipped - Tracks currently equipped trait per zone
export const userAvatarEquipped = pgTable("user_avatar_equipped", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  userIdx: index("user_avatar_equipped_user_idx").on(table.userId),
  zoneIdx: index("user_avatar_equipped_zone_idx").on(table.zoneId),
  traitIdx: index("user_avatar_equipped_trait_idx").on(table.traitId),
  uniqueUserZone: unique().on(table.userId, table.zoneId),
}));

// Environment States - Living forest progression system
export const environmentStates = pgTable("environment_states", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull().unique(),
  
  environmentLevel: integer("environment_level").default(1).notNull(),
  
  unlockedElements: json("unlocked_elements").$type<{
    trees: string[];
    animals: string[];
    decorations: string[];
    effects: string[];
  }>().default({ trees: [], animals: [], decorations: [], effects: [] }),
  
  environmentXp: integer("environment_xp").default(0),
  xpToNextLevel: integer("xp_to_next_level").default(100),
  
  currentTheme: varchar("current_theme").default("forest"),
  seasonalTheme: varchar("seasonal_theme"),
  
  lastActivityAt: timestamp("last_activity_at").defaultNow(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  userIdx: index("environment_states_user_idx").on(table.userId),
  levelIdx: index("environment_states_level_idx").on(table.environmentLevel),
}));

// Trait Reward Logs - Anti-cheat auditing for trait unlocks
export const traitRewardLogs = pgTable("trait_reward_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  userIdx: index("trait_reward_logs_user_idx").on(table.userId),
  traitIdx: index("trait_reward_logs_trait_idx").on(table.traitId),
  sourceIdx: index("trait_reward_logs_source_idx").on(table.source),
  hashIdx: index("trait_reward_logs_hash_idx").on(table.verificationHash),
  isValidIdx: index("trait_reward_logs_is_valid_idx").on(table.isValid),
  createdAtIdx: index("trait_reward_logs_created_at_idx").on(table.createdAt),
}));
...
// ===== MODULAR AVATAR ENGINE SCHEMAS & TYPES =====

// Avatar Zone Schemas
export const insertAvatarZoneSchema = createInsertSchema(avatarZones).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertAvatarTraitSchema = createInsertSchema(avatarTraits).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertUserAvatarTraitSchema = createInsertSchema(userAvatarTraits).omit({
  id: true,
  unlockedAt: true,
  createdAt: true,
});

export const insertUserAvatarEquippedSchema = createInsertSchema(userAvatarEquipped).omit({
  id: true,
  equippedAt: true,
  updatedAt: true,
});
...
export const insertTraitRewardLogSchema = createInsertSchema(traitRewardLogs).omit({
  id: true,
  createdAt: true,
});

// Rarity validation schema
export const traitRaritySchema = z.enum(['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic']);

// Unlock source validation schema  
export const unlockSourceSchema = z.enum(['achievement', 'challenge', 'purchase', 'default', 'event', 'referral']);

// Avatar Zone Types
export type AvatarZone = typeof avatarZones.$inferSelect;
export type InsertAvatarZone = z.infer<typeof insertAvatarZoneSchema>;

export type AvatarTrait = typeof avatarTraits.$inferSelect;
export type InsertAvatarTrait = z.infer<typeof insertAvatarTraitSchema>;

export type UserAvatarTrait = typeof userAvatarTraits.$inferSelect;
export type InsertUserAvatarTrait = z.infer<typeof insertUserAvatarTraitSchema>;

export type UserAvatarEquipped = typeof userAvatarEquipped.$inferSelect;
export type InsertUserAvatarEquipped = z.infer<typeof insertUserAvatarEquippedSchema>;
...
export type TraitRewardLog = typeof traitRewardLogs.$inferSelect;
export type InsertTraitRewardLog = z.infer<typeof insertTraitRewardLogSchema>;

// ===== AVATAR MARKETPLACE SYSTEM =====

// Marketplace Listings - Users can sell their avatar traits
export const marketplaceListings = pgTable("marketplace_listings", {
  ...
  traitIdx: index("marketplace_listings_trait_idx").on(table.traitId),
...
// Gift Transactions - Users can gift avatar traits to each other
export const giftTransactions = pgTable("gift_transactions", {
  ...
  receiverIdx: index("gift_transactions_receiver_idx").on(table.receiverId),
  traitIdx: index("gift_transactions_trait_idx").on(table.traitId),
  claimedIdx: index("gift_transactions_claimed_idx").on(table.claimed),
}));
...
// AI Usage Stats - Track every AI request for analytics and cost monitoring
export const aiUsageStats = pgTable("ai_usage_stats", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  
  // Request Information
  provider: varchar("provider").notNull(), // openai, gemini
  model: varchar("model").notNull(), // gpt-4o, gemini-2.0-flash, etc.
  endpoint: varchar("endpoint").notNull(), // /api/ai/chat, /api/mentor/session, etc.
  
  // Token Usage
  promptTokens: integer("prompt_tokens").default(0).notNull(),
  completionTokens: integer("completion_tokens").default(0).notNull(),
  totalTokens: integer("total_tokens").default(0).notNull(),
  estimatedCost: decimal("estimated_cost").default("0").notNull(), // USD cost estimate
  
  // Performance Metrics
  latencyMs: integer("latency_ms").notNull(), // Request duration in milliseconds
  cacheHit: boolean("cache_hit").default(false).notNull(), // Was response cached?
  errorOccurred: boolean("error_occurred").default(false).notNull(), // Did request fail?
  
  // Metadata
  timestamp: timestamp("timestamp").defaultNow().notNull(),
}, (table) => ({
  userIdIdx: index("ai_usage_stats_user_id_idx").on(table.userId),
  providerIdx: index("ai_usage_stats_provider_idx").on(table.provider),
  timestampIdx: index("ai_usage_stats_timestamp_idx").on(table.timestamp),
  errorIdx: index("ai_usage_stats_error_idx").on(table.errorOccurred),
}));

// AI Usage Stats Schemas
export const insertAIUsageStatsSchema = createInsertSchema(aiUsageStats).omit({
  id: true,
  timestamp: true,
});

// AI Usage Stats Types
...
// Community Channels - Topic-based discussion spaces
export const communityChannels = pgTable("community_channels", {
  ...
  nameIdx: index("community_channels_name_idx").on(table.name),
}));

// Community Posts - User posts within channels
export const communityPosts = pgTable("community_posts", {
  ...
  channelIdIdx: index("community_posts_channel_id_idx").on(table.channelId),
...
// Community Replies - Replies to posts
export const communityReplies = pgTable("community_replies", {
  ...
  postIdIdx: index("community_replies_post_id_idx").on(table.postId),
  authorIdIdx: index("community_replies_author_id_idx").on(table.authorId),
}));
...
export const communityPostLikes = pgTable("community_post_likes", {
  ...
  uniqueLike: unique().on(table.postId, table.userId),
  postIdIdx: index("community_post_likes_post_id_idx").on(table.postId),
  userIdIdx: index("community_post_likes_user_id_idx").on(table.userId),
}));
...
// Therapists - Mental health professionals on the platform
export const therapists = pgTable("therapists", {
  ...
  availability: json("availability").$type<{ days: string[]; startTime: string; endTime: string }>(),
  imageUrl: varchar("image_url"),
  ...
  ratingIdx: index("therapists_rating_idx").on(table.rating),
  isActiveIdx: index("therapists_is_active_idx").on(table.isActive),
}));

// Therapist Reviews - Reviews from clients
export const therapistReviews = pgTable("therapist_reviews", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  therapistId: varchar("therapist_id").references(() => therapists.id, { onDelete: 'cascade' }).notNull(),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  rating: integer("rating").notNull(),
  comment: text("comment"),
  createdAt: timestamp("created_at").defaultNow(),
}, (table) => ({
  therapistIdIdx: index("therapist_reviews_therapist_id_idx").on(table.therapistId),
  userIdIdx: index("therapist_reviews_user_id_idx").on(table.userId),
}));

// Therapist Bookings - Scheduled sessions
export const therapistBookings = pgTable("therapist_bookings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  therapistId: varchar("therapist_id").references(() => therapists.id, { onDelete: 'cascade' }).notNull(),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  ...
  therapistIdIdx: index("therapist_bookings_therapist_id_idx").on(table.therapistId),
  userIdIdx: index("therapist_bookings_user_id_idx").on(table.userId),
  scheduledAtIdx: index("therapist_bookings_scheduled_at_idx").on(table.scheduledAt),
  statusIdx: index("therapist_bookings_status_idx").on(table.status),
}));

// Therapist Insert Schemas
...
export const insertTherapistBookingSchema = createInsertSchema(therapistBookings).omit({
  id: true,
  createdAt: true,
});
...
export type TherapistBooking = typeof therapistBookings.$inferSelect;
export type InsertTherapistBooking = z.infer<typeof insertTherapistBookingSchema>;

// ===== GROWTH SANCTUARY SYSTEM =====

// Sanctuary Elements - Trees, creatures, decorations available in the sanctuary
export const sanctuaryElements = pgTable("sanctuary_elements", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: varchar("name", { length: 100 }).notNull(),
  type: varchar("type", { length: 50 }).notNull(), // tree, creature, decoration, effect
  category: varchar("category", { length: 50 }).default("nature"), // nature, magical, seasonal, legendary
  rarity: varchar("rarity", { length: 20 }).default("common"), // common, uncommon, rare, epic, legendary
  evolutionStage: integer("evolution_stage").default(1), // 1-5: Seedling, Sapling, Young Forest, Mature Forest, Ancient Grove
  unlockCost: integer("unlock_cost").default(100), // coins to unlock
  xpRequirement: integer("xp_requirement").default(0), // XP needed to unlock
  levelRequirement: integer("level_requirement").default(1), // user level needed
  description: text("description"),
  animationType: varchar("animation_type", { length: 50 }).default("idle"), // idle, sway, fly, walk, glow
  assetData: json("asset_data").$type<{
    icon?: string;
    colors?: string[];
  ...
  typeIdx: index("sanctuary_elements_type_idx").on(table.type),
  rarityIdx: index("sanctuary_elements_rarity_idx").on(table.rarity),
  evolutionStageIdx: index("sanctuary_elements_evolution_stage_idx").on(table.evolutionStage),
}));

// User Sanctuary - User's sanctuary state and unlocked elements
export const userSanctuary = pgTable("user_sanctuary", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull().unique(),
  sanctuaryName: varchar("sanctuary_name", { length: 100 }).default("My Sanctuary"),
  evolutionStage: integer("evolution_stage").default(1), // 1-5
  sanctuaryXp: integer("sanctuary_xp").default(0),
  xpToNextStage: integer("xp_to_next_stage").default(500),
  weatherType: varchar("weather_type", { length: 30 }).default("sunny"), // sunny, cloudy, rainy, aurora, starry
  timeOfDay: varchar("time_of_day", { length: 20 }).default("day"), // dawn, day, dusk, night
  unlockedElements: json("unlocked_elements").$type<string[]>().default([]),
  placedElements: json("placed_elements").$type<{
    elementId: string;
    x: number;
    y: number;
    layer: number;
  }[]>().default([]),
  totalElementsUnlocked: integer("total_elements_unlocked").default(0),
  lastVisited: timestamp("last_visited").defaultNow(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  userIdIdx: index("user_sanctuary_user_id_idx").on(table.userId),
  evolutionStageIdx: index("user_sanctuary_evolution_stage_idx").on(table.evolutionStage),
}));

// Sanctuary Evolution Stages - Requirements and rewards for each stage
export const sanctuaryEvolutionStages = pgTable("sanctuary_evolution_stages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  stage: integer("stage").notNull().unique(), // 1-5
  name: varchar("name", { length: 100 }).notNull(), // Seedling, Sapling, Young Forest, Mature Forest, Ancient Grove
  description: text("description"),
  xpRequired: integer("xp_required").notNull(), // XP needed to reach this stage
  elementsUnlocked: integer("elements_unlocked").default(0), // Elements available at this stage
  specialReward: json("special_reward").$type<{
    type: string;
    value: number;
    description?: string;
  }>(),
  themeData: json("theme_data").$type<{
    skyGradient: string;
  ...
  stageIdx: index("sanctuary_evolution_stages_stage_idx").on(table.stage),
}));

// Sanctuary Insert Schemas
export const insertSanctuaryElementSchema = createInsertSchema(sanctuaryElements).omit({
  id: true,
  createdAt: true,
});

export const insertUserSanctuarySchema = createInsertSchema(userSanctuary).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertSanctuaryEvolutionStageSchema = createInsertSchema(sanctuaryEvolutionStages).omit({
  id: true,
  createdAt: true,
});

// Sanctuary Types
export type SanctuaryElement = typeof sanctuaryElements.$inferSelect;
export type InsertSanctuaryElement = z.infer<typeof insertSanctuaryElementSchema>;

export type UserSanctuary = typeof userSanctuary.$inferSelect;
export type InsertUserSanctuary = z.infer<typeof insertUserSanctuarySchema>;

export type SanctuaryEvolutionStage = typeof sanctuaryEvolutionStages.$inferSelect;
export type InsertSanctuaryEvolutionStage = z.infer<typeof insertSanctuaryEvolutionStageSchema>;

// ===== AI RE-PLANNING ENGINE =====

// AI Re-planning Logs - Track AI-generated recommendations and their outcomes
export const aiReplanningLogs = pgTable("ai_replanning_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  
  // Target Entity
  targetId: varchar("target_id").notNull(),
  targetType: varchar("target_type").notNull(), // goal, task, habit
  
  // Recommendation Details
  recommendationType: varchar("recommendation_type").notNull(), // reschedule, simplify, break_down, motivate, pause
  priority: varchar("priority").default("medium"), // high, medium, low
  suggestion: text("suggestion").notNull(),
  reasoning: text("reasoning").notNull(),
  
  // Fogg Behavior Model Analysis (B = M × A × P)
  foggAnalysis: json("fogg_analysis").$type<{
  ...
  // Proposed Changes (JSON for flexibility)
  proposedChanges: json("proposed_changes").$type<any>(),
  
  // Status Tracking
  status: varchar("status").default("pending"), // pending, applied, dismissed, expired
  ...
  // Metadata
  aiModel: varchar("ai_model").default("gpt-4o-mini"),
  ...
  userIdIdx: index("ai_replanning_logs_user_id_idx").on(table.userId),
  targetIdx: index("ai_replanning_logs_target_idx").on(table.targetId, table.targetType),
  statusIdx: index("ai_replanning_logs_status_idx").on(table.status),
  priorityIdx: index("ai_replanning_logs_priority_idx").on(table.priority),
  createdAtIdx: index("ai_replanning_logs_created_at_idx").on(table.createdAt),
}));

// AI Re-planning Insert Schema
export const insertAIReplanningLogSchema = createInsertSchema(aiReplanningLogs).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  appliedAt: true,
...
// AI Re-planning Types
export type AIReplanningLog = typeof aiReplanningLogs.$inferSelect;
export type InsertAIReplanningLog = z.infer<typeof insertAIReplanningLogSchema>;